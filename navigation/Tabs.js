...
import { Dimensions, StyleSheet, Text, TouchableOpacity, View,  Platform, Image } from 'react-native';
            ...
            <Tab.Navigator
                screenOptions={({route})=>({
                    ...
                    tabBarOptions={{
                        ...
                        tabBarIcon: ({ focused }) => {
                            const Speaker = focused
                            ? require('../Assets/Img/speakerFocused.png')
                            : require('../Assets/Img/speakerNonFocused.png')
                        }
                    }}>
                ...
                <Tab.Screen name="Conférenciers" component={SpeakersScreen} options={{
                    tabBarIcon: ({focused}) =>{
                        const Speaker = focused
                        ? require('../Assets/Img/speakerFocused.png')
                        : require('../Assets/Img/speakerNonFocused.png')
                        return (
                        <View style={{alignItems:'center',justifyContent:'center',}}>
                            <Image source={Speaker} style={{top: focused ? stl : stli , width: focused ? 40 : 35, height: focused ? 40 : 40,}} />
                ...
                <Tab.Screen name="Com" component={ComScreen} options={{
                    tabBarIcon: ({focused}) =>(
                        <View style={{alignItems:'center',justifyContent:'center'}}>
                            <Icon name="easel" size={focused ? 35 : 29} style={{top: focused ? stl : stli,paddingLeft:5, color: focused ? '#d60115': '#748c94'}} />
                            <Text style={{top:PhoneSize(), color: focused ? '#d60115': '#748c94', fontFamily:'montserratlight', fontSize: 15, opacity: focused ? 1 : 0}}>Posters</Text>
                        </View>
                    ),
                    tabBarButton: (props) =>(
                        <TouchableOpacity {...props} />
